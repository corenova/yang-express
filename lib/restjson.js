...
const restjson = (opts = {}) => {
  ...
  const router = Router();
  const bp = bodyparser;
  const mp = multipart({ dest: UPLOAD_DEST });
  ...
  router.use(bp.urlencoded({ extended: true, limit }),
	     bp.json({ strict: true, type:'*/json', limit }));
  ...
  router.route('*')
    ...
    .post(mp.any(), autoreap, (req, res, next) => {
      const { match } = res.locals
	...
	ctx = ctx.with({ user: req.user });
      ...
      case 'rpc':
        if (req.files) {
	  res.on('autoreap', (file) => ctx.logDebug(`auto-reaped: ${file}`));
          req.body = req.files.reduce((a,b) => {
            a[b.fieldname] = b.path;
            return a;
          },{});
        }
        ctx.push(req.body)
    ...
    .put(transact((ctx, data) => ctx.push(data))) // replicate same behavior as PATCH
