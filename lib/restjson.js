  ...
  const transact = (callback) => {
    ...
    return (req, res, next) => {
      ...
      if (Array.isArray(match)) {
        res.send(match.map((x) => callback.call(x, req.body).toJSON()))
      } else {
        res.send(callback.call(match, req.body).toJSON())
      }
    }
  }
  ...
  router.route('*')
    ...
    .post((req, res, next) => {
      ...
      case 'rpc':
        ...
        break;
      case 'list':
        res.status(201).send(match.create(req.body).toJSON())
        break;
      default:
        res.status(400).end()
      }
