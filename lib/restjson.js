...
const restjson = (opts = {}) => {
  ...
  const transact = (callback) => {
    ...
    return async (req, res, next) => {
      ...
      const state = passport ? { session: req.user.token } : {}
      if (Array.isArray(match)) {
	const results = await Promise.all(match.map(m => callback(m.context.with(state), req.body)));
	res.send(results.map(x => x.toJSON(true)));
      } else {
	const result = await callback(match.context.with(state), req.body);
  ...
  router.route('*')
    ...
    .post(mp.any(), (req, res, next) => {
      ...
      if (passport) {
	ctx.with({ session: req.user.token });
      }	    
      switch (match.kind) {
      case 'action':
      case 'rpc':
        ...
        break;
      case 'list':
	ctx.push(req.body, { createOnly: true })
	  .then(list => res.status(201).send({ [ctx.name]: list.change }))
	  .catch(err => next(err))
        break;
      default:
        res.status(400).end()
    ...
    .put(transact((ctx, data) => ctx.push(data, { replace: true })))
